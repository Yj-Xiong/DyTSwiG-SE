...
**Abstract:** 
Speech enhancement (SE) models based on deep denoising networks (DDNs) typically stack multiple Two-Stage (TS) blocks to progressively refine the input signal, structuring the denoising task as progressive learning process. These models achieve impressive performance, but face a trade-off between performance and efficiency. Furthermore, widely adopted phase decoders tend to ignore the noisy phase condition. In response, we introduce two layer-normalization-free monaural SE networks: DyTSwiG-Net and DyTSwiG-Mamba. First, we introduce the SwiGLUformer as a more efficient replacement for the TS block, along with an Input-Biased Dynamic Tanh (IB-DyT) activation for layer normalization-free architectures. For DyTSwiG-Mamba, we further design a Dual-Branch Phase Decoder (DBPD) to jointly estimate phase mapping and masking, and integrate a Global Bidirectional Mamba (G-BiMamba) module to enhance feature aggregation across the network.
Both models are thoroughly evaluated on four datasets in English and Mandarin. Ablation studies and visual analysis verify the effectiveness of the SwiGLUformer and DBPD. Experimental results show that DyTSwiG-Net achieves faster inference than existing models while maintaining competitive performance Notably, DyTSwiG-Mamba surpasses the state-of-the-art methods while saving around 25% of overall costs. 
