...
class TFFMambaBlock(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # Initialize ConvTranspose1d layers
        self.tlinear = nn.ConvTranspose1d(self.hid_feature * 1, self.hid_feature, 1, stride=1)
        self.flinear = nn.ConvTranspose1d(self.hid_feature * 1, self.hid_feature, 1, stride=1)
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 3, 2, 1).contiguous().view(b * f, t, c)
        x_f1 = self.tlinear(self.time_mamba(x).permute(0, 2, 1)).permute(0, 2, 1)  + x #* self.t_weight
        #  x = x.view(b,c,t,f)
        # x = self.T_SRU(x)
        x = x_f1.view(b, f, t, c).permute(0, 2, 1, 3).contiguous().view(b * t, f, c)
        x_f2 = self.flinear(self.freq_mamba(x).permute(0, 2, 1)).permute(0, 2, 1) + x #* self.f_weight
