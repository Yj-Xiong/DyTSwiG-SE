...
from models.generator_DyTSwiGNet import LKFCA_Net as LKFCA_Net
from models.g_3090 import pesq_score, phase_losses
from models.discriminator import MetricDiscriminator, batch_pesq, batch_ssnr
...
# class adaptive_time_loss():
#     def __init__(self, threshold=10.5):
#         self.learning_weight = nn.Parameter(torch.rand(1).clamp_(0.7, 1),requires_grad=True)
#         self.SSNR_Threshold = threshold
#     def forward(self, clean,enhanced):
# 将单个音频张量转为WAV字节流
def tensor_to_wav(waveform: torch.Tensor, sample_rate: int) -> bytes:
    # 转为CPU和双通道（torchaudio需要[batch, channels, length]）
    waveform = waveform.unsqueeze(0)  # [1, datalength] -> [1, 1, datalength]
...
# 示例：转换批次中的第一个音频
# wav_bytes = tensor_to_wav_bytes(audio_tensor[0], sample_rate=16000)
def train(rank, a, h):
    if h.num_gpus > 1:
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            metric_r = discriminator(clean_mag, clean_mag)
            metric_g = discriminator(clean_mag, mag_g_con.detach())
            ...
            # consis Complex Loss
            loss_consis_con = F.mse_loss(com_g, com_g_con)
            # Time Loss
            ...
            if rank == 0:
                ...
                if steps % a.stdout_interval == 0:
                    with torch.no_grad():
                        ...
                        consis_con_error = F.mse_loss(com_g, com_g_con).item()
                        time_error = F.l1_loss(clean_audio, audio_g).item()
                        #cssnr_error = loss_lcsnr.item()
                    print(
                        'Steps : {:d}, Gen Loss: {:4.3f}, Disc Loss: {:4.3f}, Metric loss: {:4.3f}, Magnitude Loss : {:4.3f}, Phase Loss : {:4.3f}, Complex Loss : {:4.3f}, Con_Complex Loss : {:4.3f}, Time Loss : {:4.3f}, s/b : {:4.3f}'.
                        format(steps, loss_gen_all, loss_disc_all, metric_error, mag_error, pha_error, com_error,
                               consis_con_error, time_error, time.time() - start_b))

                # checkpointing
                if steps % a.checkpoint_interval == 0 and steps != 0 or steps % len(
                        train_loader) == 0 and steps != 0:
                    ...
                    save_checkpoint(checkpoint_path,
                                    {'discriminator': (
                                        discriminator.module if h.num_gpus > 1 else discriminator).state_dict(),
                ...
                if steps % a.summary_interval == 0:
                    ...
                    sw.add_scalar("Training/Con_Complex Loss", consis_con_error, steps)
                    sw.add_scalar("Training/Time Loss", time_error, steps)

                # Validation
                if steps % a.validation_interval == 0 and steps != 0 or steps % len(
                        train_loader) == 0 and steps != 0:
                    ...
                    if epoch >= a.best_checkpoint_start_epoch:
                        if val_pesq_score > best_pesq:
                            ...
                            best_checkpoint_path = "{}/g_best_PESQ{}_epoch{}".format(a.checkpoint_path, val_pesq_score,
                                                                                     epoch)
...
def main():
    ...
    #parser.add_argument('--input_clean_wavs_dir', default='/home/xyj/Experience/MP-SENet/VoiceBank+DEMAND/wav_clean')
    #parser.add_argument('--input_noisy_wavs_dir', default='/home/xyj/Experience/MP-SENet/VoiceBank+DEMAND/wav_noisy')
    #parser.add_argument('--input_training_file', default='/home/xyj/Experience/MP-SENet/VoiceBank+DEMAND/training.txt')
    #parser.add_argument('--input_validation_file', default='/home/xyj/Experience/MP-SENet/VoiceBank+DEMAND/test.txt')
    parser.add_argument('--group_name', default=None)
    parser.add_argument('--input_clean_wavs_dir', default='/home/xyj/datasets/chinese/clean_file')
    parser.add_argument('--input_noisy_wavs_dir', default='/home/xyj/datasets/chinese/noisy_file')
